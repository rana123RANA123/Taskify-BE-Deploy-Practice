...
const { errored } = require('../config/logger');
...
const resetPassword = async (resetPasswordToken, newPassword) => {
  try {
    ...
    console.log('Reset password token document:', resetPasswordTokenDoc);
    const user = await userService.getUserById(resetPasswordTokenDoc.user);
    if (!user) {
      console.error('User not found for token document:', resetPasswordTokenDoc);
      throw new Error();
    }
